            ...
            # if not self.xero_tax_type_id:
                    if t.xero_record_taxtype:
                        if t.xero_record_taxtype == 'REVERSECHARGES':
                            raise Warning(' The Tax with this ReportTaxType is not yet available for create and update via the API. They are returned on GET requests only')
                        if t.xero_record_taxtype == 'NONE':
                            raise Warning(' The Tax with this ReportTaxType is used for system tax rates and only returned on GET requests')
                        ...
                        for element in response_data.get('Elements'):
                            if element.get('ValidationErrors'):
                                for err in element.get('ValidationErrors'):
                                    raise Warning('(Tax) Xero Exception : ' + err.get('Message'))
