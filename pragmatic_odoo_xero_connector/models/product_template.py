...
class ProductProduct(models.Model):
    ...
    def prepare_product_export_dict(self):
        ...
        # if self.taxes_id:
        #
        #     cust_id = self.env['account.tax'].get_xero_tax_ref(self.taxes_id)
        #     print("cust_id.xero_tax_type_id 1------------------> ", cust_id)
        #     if cust_id:
        #         tax_id = {
        #             'TaxType': cust_id.xero_tax_type_id
        #         }
        # else:
        #     tax_id = {
        ...
        #     cust_id = self.env['account.tax'].get_xero_tax_ref(self.supplier_taxes_id)
        #     print("cust_id.xero_tax_type_id 2 ------------------> ", cust_id)
        #     if cust_id:
        #         tax_id_purchase = {
        #             'TaxType': cust_id.xero_tax_type_id
        #         }
            ...
            if self.property_account_expense_id.xero_account_id:
                ...
                if self.type == 'product':
                    purchase_dict = {
                        "UnitPrice": standard_price,
                        "COGSAccountCode": self.property_account_expense_id.code,
                        # "COGSAccountCode": self.categ_id.property_account_income_categ_id.code
                    }
                else:
        ...
        if self.property_account_income_id:
            ...
            if self.categ_id.property_account_income_categ_id.xero_account_id:
                # print("3 ------------> ", self.categ_id.property_account_income_categ_id.xero_account_id)
                sales_dict = {
                    "UnitPrice": list_price,
                    "AccountCode": self.categ_id.property_account_income_categ_id.code,
                }
            else:
                # print("4 ------------> ", self.categ_id.property_account_income_categ_id)
        ...
        if self.default_code:
            ...
            raise Warning('Please enter Internal Reference for ' + self.name)
                        ...
                        for element in response_data.get('Elements'):
                            if element.get('ValidationErrors'):
                                for err in element.get('ValidationErrors'):
                                    raise Warning('(Products) Xero Exception : ' + err.get('Message'))
                    elif response_data.get('Message'):
                            raise Warning(
                                '(Products) Xero Exception : ' + response_data.get('Message'))
                    else:
                        raise Warning('(Products) Xero Exception : please check xero logs in odoo for more details')
